...
from utils import MAX_WORD_SIZE
...
class Ambig(GraphPartBase):
    ...
    def __build_graph_for_device__(self, x, x_seq_len, main_probs, seq_len, net_tags, gram_drop, main_drop):
        ...
        #    _, word_result = tfu.build_rnn(rnn_input,
        #                                1,
        #                                seq_len,
        #                                self.settings['word_rnn'],
        #                                for_usage=self.for_usage,
        #                                with_seq=True)
        #    word_result = tf.reshape(word_result, (sent_batch_size, sent_max_length, self.settings['word_rnn']['rnn_state_size']))
        ...
        tag = ad_tag #tf.concat([ad_tag, net_tags], axis=1)
        tag = tf.reshape(tag, (sent_batch_size, sent_max_length, self.settings['ad_tags_max_count']))
        rnn_input = tf.concat([
            #word_result,
            main_probs,
        ...
        self.create_accuracy_metric(1, labels, result)
